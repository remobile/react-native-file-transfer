...
@synthesize activeTransfers, _totalPayloadLength;
...
- (NSURLRequest*)requestForUploadCommand:(CDVInvokedUrlCommand*)command fileData:(NSData*)fileData
{
    ...
    self._totalPayloadLength = totalPayloadLength;
    if (chunkedMode) {
...
- (void)updateProgress
{
    if (self.direction == CDV_TRANSFER_DOWNLOAD) {
        ...
        [self.command sendEventWithName:[NSString stringWithFormat:@"DownloadProgress-%@", self.objectId] body:downloadProgress];
    }
}
...
- (void)connection:(NSURLConnection*)connection didSendBodyData:(NSInteger)bytesWritten totalBytesWritten:(NSInteger)totalBytesWritten totalBytesExpectedToWrite:(NSInteger)totalBytesExpectedToWrite
{
    if (self.direction == CDV_TRANSFER_UPLOAD) {
        ...
        [uploadProgress setObject:[NSNumber numberWithLongLong:self.chunkedMode?self.command._totalPayloadLength:totalBytesExpectedToWrite] forKey:@"total"];
        [self.command sendEventWithName:[NSString stringWithFormat:@"UploadProgress-%@", self.objectId] body:uploadProgress];
    }
